...
// ✅ Import our hook — no hardcoded emails
...
const studentLoginSchema = z.object({
  ...
  roll_number: z
    ...
    .length(7, 'Roll number must be 7 digits'),
  password: z
    .string()
...
const Login: React.FC = () => {
  ...
  const getSchema = () => {
    switch (userType) {
      case 'student': return studentLoginSchema;
      case 'faculty': return facultyLoginSchema;
      case 'admin':   return adminLoginSchema;
    }
  };
  ...
  // Watch email for live hint (faculty/admin only)
  const emailValue: string = userType !== 'student' ? (watch('email') ?? '') : '';
  ...
  const getFacultyEmailHint = (): {
    ...
    if (!emailValue || userType !== 'faculty') return { type: null, message: '' };
  ...
  const handleGoogleSignIn = async () => {
    if (userType === 'faculty') {
      toast.error('Faculty must sign in with their institutional email and password.');
      return;
    }
    if (userType === 'student') {
    ...
    try {
      ...
      if (user.email?.endsWith(FACULTY_EMAIL_DOMAIN)) {
        toast.error('Faculty must use the Faculty tab and sign in with email & password.');
        await auth.signOut();
        return;
      }

      toast.success('Google sign-in successful!');
      // Navigation is handled by AuthContext observer
    } catch (error: any) {
  ...
  const onSubmit: SubmitHandler<any> = async (data) => {
    // Extra guard for faculty email format
      ...
      >
        ...
        >
          {(['student', 'faculty', 'admin'] as const).map((type) => {
            const icons: Record<UserType, JSX.Element> = {
          ...
          <div className="space-y-5">
                ...
                <div>
                  ...
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      <Hash
                        className={`h-5 w-5 ${
                          errors.roll_number ? 'text-red-500' : 'text-gray-400'
                        }`}
                      />
                    </div>
                    <input
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    Enter your 7-digit roll number
                  </p>
                </div>

                {/* Student first-time info box */}
            ...
            {(userType === 'faculty' || userType === 'admin') && (
              <div>
                ...
                {/* Live hint — only when no zod error, faculty tab only */}
            ...
            {/* ── Google Sign-In — admin only (not student, not faculty) ── */}
            {userType === 'admin' && (
              <>
                <div className="relative">
