...
class SubjectDefinition(BaseModel):
    ...
    elective_group: str | None = None  # For PEC grouping
...
# Autonomy Curriculum (2024+ for Sem 5-8, also applies to all semesters for 2025+ batches)
...
def get_semester_subjects(semester: int, admission_year: int, current_year: int = 2025) -> List[SubjectDefinition]:
    """
    Get subjects for a semester based on admission year and current curriculum
    
    Args:
        semester: Semester number (1-8)
        admission_year: Year of admission (e.g., 2022, 2023, 2024, 2025)
        current_year: Current year (default 2025)
    
    Returns:
        List of SubjectDefinition for that semester
    """
    # Determine if pre-autonomy or autonomy
    # Pre-autonomy: Admission before 2024, Semesters 1-4
    # Autonomy: Admission 2024+, All semesters OR Admission before 2024, Semesters 5-8
    
    is_pre_autonomy_batch = admission_year < 2024
    
    if is_pre_autonomy_batch and semester <= 4:
        # Use pre-autonomy curriculum for first 4 semesters
        return PRE_AUTONOMY_CURRICULUM.get(semester, [])
    else:
        # Use autonomy curriculum for sem 5+ or all semesters for new batches
