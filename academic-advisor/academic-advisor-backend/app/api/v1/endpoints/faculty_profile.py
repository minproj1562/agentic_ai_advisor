...
async def reupload_cv(
    cv: UploadFile = File(...),
    merge_mode: str = "smart",  # "smart", "replace", "append"
    current_user: FirebaseUser = Depends(get_current_user)
):
    """
    Re-upload CV for existing profile.
    merge_mode:
        - "smart": Only fill empty fields from new CV
        - "replace": Replace all fields with new CV data
        - "append": Append new data to lists (research areas, skills, etc.)
    """
    try:
        # Validate file
        if not cv.content_type == "application/pdf":
        ...
        # Parse new CV
        parsed_result = await enhanced_cv_parser.parse(content, cv.filename)
        suggested_profile = parsed_result.get('suggested_profile', {})
        ...
        try:
            ...
            cv_url = await firebase_manager.upload_file(
                ...
                file_data=content,
                content_type="application/pdf",
                metadata={"user_id": user_id, "uploaded_at": datetime.utcnow().isoformat()}
            )
        except Exception as e:
            logger.warning(f"Firebase upload failed: {e}")
        
        # Update CV metadata
        faculty.cv_url = cv_url or faculty.cv_url
        faculty.cv_file_name = cv.filename
        faculty.cv_uploaded_at = datetime.utcnow()
        faculty.cv_parsed_data = parsed_result
        ...
        if merge_mode == "replace":
            # Replace uniform profile entirely
            faculty.uniform_profile = suggested_profile
            faculty.profile_setup_complete = False  # Require review
        elif merge_mode == "append":
            # Append to list fields
            if faculty.uniform_profile and suggested_profile:
                up = faculty.uniform_profile
                sp = suggested_profile
                
                # Append research areas
                existing_areas = set(up.research_expertise.primary_areas)
                new_areas = sp.research_expertise.primary_areas
                up.research_expertise.primary_areas = list(existing_areas.union(new_areas))[:5]
                
                # Append keywords
                existing_kw = set(up.research_expertise.keywords)
                new_kw = sp.research_expertise.keywords
                up.research_expertise.keywords = list(existing_kw.union(new_kw))
                
                # Append subjects
                existing_subj = set(up.teaching.current_subjects)
                new_subj = sp.teaching.current_subjects
                up.teaching.current_subjects = list(existing_subj.union(new_subj))
                
                faculty.uniform_profile = up
        # "smart" mode - don't modify existing profile, just provide suggestions
        ...
        logger.error(f"CV reupload error: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"CV reupload failed: {str(e)}")
