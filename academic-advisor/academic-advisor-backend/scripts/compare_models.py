...
Usage:
    ...
    python -m scripts.compare_models --samples 1000 --noise 0.5
"""
...
def parse_args():
    ...
    parser.add_argument('--noise', type=float, default=0.4,
                        help='Noise level in synthetic data (default: 0.4)')
...
def generate_hard_samples(n_per_class: int = 100) -> list:
    ...
    for _ in range(n_per_class):
        ...
        # Make them also strong in networks
        ...
        for subj in sample['marks']:
            sample['marks'][subj] = random.uniform(55, 75)  # All average
        hard_samples.append(sample)
...
async def main():
    ...
    # Ask user if they want to train with the best model
    logger.info("─" * 80)
